...
useEffect(() => {
  ...
  const urgent = (briefing.urgent || []).map(i => `
    <div class="item urgent">
      <div class="item-tag">URGENT</div>
      <div class="item-title">${i.title || ""}</div>
      <div class="item-detail">${i.detail || ""}</div>
      ${i.action ? `<div class="item-action">→ ${i.action}</div>` : ""}
    </div>`).join("");

  const watch = (briefing.watch || []).map(i => `
    <div class="item watch">
      <div class="item-tag">WATCH</div>
      <div class="item-title">${i.title || ""}</div>
      <div class="item-detail">${i.detail || ""}</div>
    </div>`).join("");

  const healthy = (briefing.healthy || []).map(i => `
    <div class="item healthy">
      <div class="item-tag">HEALTHY</div>
      <div class="item-title">${i.title || ""}</div>
      <div class="item-detail">${i.detail || ""}</div>
    </div>`).join("");
  ...
  const insightHtml = insights.map((ins, i) => `
    <div class="insight-item">
      ...
      <div>
        ...
        <div class="insight-meta">Confidence: ${ins.confidence || ""} · Source: ${ins.source || ""}</div>
      </div>
    </div>`).join("");

  const anomalyHtml = anomalies.map(a => `
    <div class="anomaly-item ${a.severity}">
      <div class="anomaly-sev">${(a.severity || "").toUpperCase()}</div>
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: Arial, sans-serif; background: #050508; color: #F0F4FF; padding: 48px; }
    .header { border-bottom: 2px solid #FF1E3C; padding-bottom: 24px; margin-bottom: 32px; }
    .zevo-name { font-size: 48px; font-weight: 900; color: #FF1E3C; letter-spacing: -2px; }
    .zevo-sub { font-size: 13px; color: #7A8499; letter-spacing: 3px; margin-top: 4px; }
    .date { font-size: 13px; color: #7A8499; margin-top: 8px; }
    .rows { font-size: 13px; color: #FF1E3C; margin-top: 4px; }
    .section { margin-bottom: 32px; }
    .section-title { font-size: 10px; font-weight: 800; letter-spacing: 4px; color: #FF1E3C; text-transform: uppercase; margin-bottom: 14px; padding-bottom: 6px; border-bottom: 1px solid rgba(255,30,60,0.2); }
    .item { background: #0C0C18; border-radius: 8px; padding: 16px; margin-bottom: 10px; border-left: 3px solid #444; }
    .item.urgent { border-left-color: #FF1E3C; }
    .item.watch { border-left-color: #FFB800; }
    .item.healthy { border-left-color: #00FF88; }
    .item-tag { font-size: 9px; font-weight: 800; letter-spacing: 2px; color: #7A8499; margin-bottom: 6px; }
    .item.urgent .item-tag { color: #FF1E3C; }
    .item.watch .item-tag { color: #FFB800; }
    .item.healthy .item-tag { color: #00FF88; }
    .item-title { font-size: 14px; font-weight: 700; margin-bottom: 6px; }
    .item-detail { font-size: 12px; color: #7A8499; line-height: 1.7; }
    .item-action { font-size: 12px; color: #FF1E3C; font-weight: 600; margin-top: 6px; }
    .kpi-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
    .kpi-item { background: #0C0C18; border-radius: 8px; padding: 16px; }
    .kpi-label { font-size: 10px; color: #7A8499; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 6px; }
    .kpi-value { font-size: 22px; font-weight: 900; color: #F0F4FF; margin-bottom: 4px; }
    .kpi-context { font-size: 11px; color: #4A5568; }
    .insight-item { display: flex; gap: 16px; background: #0C0C18; border-radius: 8px; padding: 16px; margin-bottom: 10px; }
    .insight-num { font-size: 18px; font-weight: 900; color: #FF1E3C; flex-shrink: 0; width: 30px; }
    .insight-title { font-size: 13px; font-weight: 700; margin-bottom: 4px; }
    .insight-detail { font-size: 12px; color: #7A8499; line-height: 1.7; }
    .insight-meta { font-size: 11px; color: #3D4560; margin-top: 6px; }
    .anomaly-item { background: #0C0C18; border-radius: 8px; padding: 16px; margin-bottom: 10px; border-left: 3px solid #FFB800; }
    .anomaly-item.critical { border-left-color: #FF1E3C; }
    .anomaly-sev { font-size: 9px; font-weight: 800; color: #FFB800; letter-spacing: 2px; margin-bottom: 6px; }
    .anomaly-item.critical .anomaly-sev { color: #FF1E3C; }
    .anomaly-title { font-size: 13px; font-weight: 700; margin-bottom: 4px; }
    .anomaly-detail { font-size: 12px; color: #7A8499; line-height: 1.7; }
    .story-box { background: #0C0C18; border-radius: 8px; padding: 20px; font-size: 14px; color: #C8D0E0; line-height: 1.9; font-style: italic; }
    .top-action { background: rgba(255,30,60,0.08); border: 1px solid rgba(255,30,60,0.3); border-radius: 8px; padding: 20px; margin-bottom: 32px; }
    .top-action-label { font-size: 9px; font-weight: 800; color: #FF1E3C; letter-spacing: 3px; margin-bottom: 8px; }
    .top-action-text { font-size: 15px; font-weight: 700; margin-bottom: 8px; }
    .top-action-impact { font-size: 12px; color: #7A8499; }
    .footer { margin-top: 48px; padding-top: 16px; border-top: 1px solid rgba(255,30,60,0.15); font-size: 11px; color: #3D4560; text-align: center; }
    @media print { body { background: #050508 !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; } }
  </style>
</head>
<body>
  <div class="header">
    <div class="zevo-name">ZEVO</div>
    <div class="zevo-sub">BUSINESS PHYSICS ENGINE</div>
    <div class="date">${new Date().toLocaleDateString("en-IN", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</div>
    <div class="rows">${(analysis?.data_stats?.total_rows || 0).toLocaleString()} rows analysed · Data quality: ${confidence?.overall || 0}%</div>
  </div>

  ${briefing.greeting ? `<div class="section"><p style="font-size:16px;color:#7A8499;font-style:italic;">${briefing.greeting}</p></div>` : ""}

  ${briefing.top_action ? `
  <div class="top-action">
    <div class="top-action-label">TODAY'S TOP ACTION</div>
    <div class="top-action-text">${briefing.top_action.action || ""}</div>
    <div class="top-action-impact">Estimated impact: ${briefing.top_action.impact || ""}</div>
  </div>` : ""}

  <div class="section">
    <div class="section-title">Morning Brief</div>
    ${urgent}${watch}${healthy}
  </div>
  ...
  <div class="section">
    ...
    <div class="story-box">${analysis.data_story}</div>
  </div>` : ""}

  <div class="footer">
    Generated by ZEVO Business Physics Engine &nbsp;·&nbsp; Employees work 9 to 5. ZEVO works 5 to 9.
  </div>
